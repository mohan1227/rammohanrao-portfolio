import { Drawer, List, ListItem, ListItemButton, ListItemIcon, ListItemText, Box, IconButton, useTheme, useMediaQuery } from '@mui/material';
import { Home, Person, Code, Work, School, EmojiEvents, Mail, Close, GitHub, LinkedIn, Twitter } from '@mui/icons-material';
...
export const Sidebar = () => {
  ...
  const drawerContent = (
    ...
    >
      ...
      <Box sx={{ p: 3, textAlign: 'center', position: 'relative', zIndex: 1 }}>
        ...
        <Box sx={{ fontSize: '0.875rem', color: 'text.secondary', mt: 0.5 }}>Assistant Professor</Box>
      </Box>

      <List ref={listRef} sx={{ flex: 1, px: 2, position: 'relative', zIndex: 1 }}>
        {menuItems.map((item) => (
  ...
  return (
    <>
      {isMobile ? (
        <Drawer anchor="left" open={isOpen} onClose={() => dispatch(closeSidebar())}>
          {drawerContent}
        </Drawer>
      ) : (
        <Drawer
